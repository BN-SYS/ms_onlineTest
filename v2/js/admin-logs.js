...
function openDetail(sessionId) {
    const sessionLogs = logsDB.filter(log => log.sessionId === sessionId);
    
    // 세션 내 모든 로그 표시 (여러 번 응시 포함)
    const detailContent = `
        <div style="padding: 30px;">
            <h3>세션 로그: ${sessionId}</h3>
            ${sessionLogs.map((log, index) => `
                <div style="background: var(--bg-gray); padding: 20px; border-radius: 8px; margin-bottom: 15px;">
                    <h4>시도 ${index + 1}</h4>
                    <p>단계: ${log.stage}단계</p>
                    <p>완료 여부: ${log.completed ? '✓ 완료' : '✗ 미완료'}</p>
                    ${log.reason ? `<p>사유: ${log.reason}</p>` : ''}
                    <p>정답 수: ${log.correctCount}</p>
                    <p>소요 시간: ${Math.floor(log.totalTime / 60)}분 ${log.totalTime % 60}초</p>
                    
                    <details style="margin-top: 15px;">
                        <summary style="cursor: pointer; font-weight: 600;">문제별 상세</summary>
                        <div style="margin-top: 10px;">
                            ${log.answers ? log.answers.map((ans, i) => `
                                <p>문제 ${i+1}: ${ans.questionId} → 답변: ${ans.choiceId} (${Math.round((ans.endTime - ans.startTime)/1000)}초)</p>
                            `).join('') : '데이터 없음'}
                        </div>
                    </details>
                </div>
