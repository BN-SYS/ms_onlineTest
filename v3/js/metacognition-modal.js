...
class TestManager {
  constructor(config) {
    ...
    this.elapsedTime = 0;
    this.tempStageResult = null; // ✅ 3단계 메타인지용 임시 저장
  }
  ...
  submitMetacognition() {
    ...
    // ✅ 임시 저장된 결과를 사용하여 최종 저장
    if (this.tempStageResult) {
      const { correctCount, correctRate, totalTime, avgTimePerQuestion, answeredCount } = this.tempStageResult;
      this.saveStageResult(correctCount, correctRate, totalTime, avgTimePerQuestion, answeredCount);
    } else {
      // 만약을 대비한 fallback
      console.error('⚠️ tempStageResult가 없습니다!');
  ...
  processSubmission() {
    ...
    // ✅ 검증 통과 후: 3단계인 경우에만 메타인지 모달 표시
    if (this.config.stage === 3) {
      console.log('✅ 3단계 검증 통과 - 메타인지 모달 표시');
      
      // 임시로 결과 데이터 저장 (메타인지 완료 후 최종 저장)
      this.tempStageResult = {
        correctCount,
        correctRate: parseFloat(correctRate.toFixed(1)),
        totalTime: actualTotalTime,
        avgTimePerQuestion: parseFloat(avgTimePerQuestion.toFixed(1)),
    ...
    // ✅ 1·2단계 또는 검증 통과: 결과 저장 및 다음 페이지 이동
    this.saveStageResult(correctCount, correctRate, actualTotalTime, avgTimePerQuestion, answeredCount);
  }

  /* ========== ✅ 단계 결과 저장 및 페이지 이동 (메서드 분리) ========== */
  saveStageResult(correctCount, correctRate, actualTotalTime, avgTimePerQuestion, answeredCount) {
    const stageResult = {
      stage: this.config.stage,
